...
typedef SafeZoneMapBuilder =
    Widget Function(BuildContext context, Geofence zone);
    ...
    return Column(
      ...
      children: [
        ...
        if (zone == null)
          _ZoneSurface(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Save home, school or another familiar place, then choose its boundary on the map.',
                  style: _body(context),
                ),
              ],
            ),
          ...
          Text(
            '$active active · ${widget.zones.length} saved',
            style: _body(context),
          ),
              ...
              children: [
                for (final item in widget.zones)
                  ChoiceChip(
                    ...
                    label: Text(
                      '${item.name} · ${deviceForZone(item, widget.devices)?.displayName ?? 'Unlinked watch'}',
                    ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(
                      height: wide ? 370 : 250,
                      child:
                          widget.mapBuilder?.call(context, zone) ??
                          SafeZoneMap(zone: zone),
                    ),
                    Padding(
                      padding: const EdgeInsets.symmetric(
                        horizontal: 12,
                      ...
                      child: Align(
                        ...
                        child: TextButton.icon(
                          onPressed: SafeZoneMapGeometry.fromZone(zone) == null
                              ? null
                              : () => widget.onExpand(zone),
                          icon: const Icon(
...
class _ZoneDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final radiusLabel = radius.isFinite && radius > 0
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  zone.active ? 'Zone active' : 'Zone paused',
          ...
          _ZoneFact(
            ...
            title: 'Assigned to',
            value: device?.displayName ?? 'Linked watch unavailable',
          ),
          const SizedBox(height: 16),
          _ZoneFact(
          ...
          Text(
            zone.active
                ? 'Arrival and departure alerts use this saved boundary.'
          ...
          if (alertsUnavailable)
            Text(
              'Alert history is unavailable. Try reopening Safe zones.',
              style: _body(context),
            )
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 6),
                    Text(
                      'An alert for this assigned watch still needs review. This does not establish its current location.',
                      style: _body(context),
                    ),
                    TextButton.icon(
                      onPressed: onAlerts,
            ...
            Text(
              latest == null
                  ...
                  : '${latest.type == 'geofence_enter' ? 'Arrival recorded' : 'Departure recorded'} · ${_eventTime(latest.createdAt!)}',
              style: _body(context),
            ),
          ],
          const SizedBox(height: 20),
          OutlinedButton.icon(
            ...
            icon: Icon(
              zone.active ? Icons.pause_rounded : Icons.play_arrow_rounded,
              size: 20,
            ),
            label: Text(
              busy
  ...
  return await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Text(
              'Remove "${zone.name}" and stop future arrival and departure alerts for it? You can pause the zone instead to keep its saved boundary.',
            ),
          ),
          actions: [
            TextButton(
