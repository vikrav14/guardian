...
class _WellnessChartState extends State<WellnessChart> {
  ...
  int? get _selected {
    final index = widget.points.indexWhere((p) => p.at == _selectedAt &&
        p.value == _selectedValue && p.secondary == _selectedSecondary);
        ...
        Focus(
          ...
          onKeyEvent: (_, event) {
            ...
            final direction = event.logicalKey == LogicalKeyboardKey.arrowRight
                ? 1
                : event.logicalKey == LogicalKeyboardKey.arrowLeft ? -1 : 0;
            if (direction == 0) return KeyEventResult.ignored;
            _select(((selected ?? (direction > 0 ? -1 : widget.points.length)) +
                direction).clamp(0, widget.points.length - 1).toInt());
          ...
          child: Semantics(
            ...
            hint: 'Use left and right arrow keys to inspect records, or open the log below.',
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                void inspect(Offset offset) {
                  ...
                  for (var i = 0; i < widget.points.length; i++) {
                    final delta = (layout.x(widget.points[i].at) - offset.dx).abs();
                    if (delta < distance) {
                      nearest = i;
                      distance = delta;
                    }
...
class _PlotLayout {
  _PlotLayout({
    ...
    final values = points.expand((p) => [p.value, p.secondary])
    ...
    final padding = activity ? 0.0 : math.max((maximum - minimum) * .15,
        maximum.abs() < 60 ? .1 : 1.0);
    ...
    ticks = [for (var i = 0; i <= ((high - low) / step).round(); i++) low + i * step];
    final widest = ticks.map((v) => measure(number(v)).width)
        .fold<double>(0, math.max);
    final fontHeight = measure('12 Sep').height;
    plot = Rect.fromLTRB(widest + 12, fontHeight + 12,
        math.max(widest + 32, size.width - 14), size.height - fontHeight * 2 - 18);
  }
  ...
  TextPainter measure(String value) => TextPainter(
    text: TextSpan(text: value, style: style), textDirection: TextDirection.ltr,
  )..layout();
  String number(double value) => NumberFormat.decimalPatternDigits(
    decimalDigits: digits,
  ).format(value);
  double x(DateTime at) => plot.left +
      at.difference(window.start).inMilliseconds /
      window.end.difference(window.start).inMilliseconds * plot.width;
...
class _WellnessPainter extends CustomPainter {
  ...
  void label(Canvas canvas, String value, Offset at, {double align = 0}) {
    ...
    final dx = (at.dx - text.width * align).clamp(0.0,
        math.max(0.0, layout.size.width - text.width)).toDouble();
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final grid = Paint()..color = colors.border..strokeWidth = 1;
    label(canvas, unit, Offset(plot.left, 0));
    for (final tick in layout.ticks) {
      final y = layout.y(tick);
      canvas.drawLine(Offset(plot.left, y), Offset(plot.right, y), grid);
      label(canvas, layout.number(tick),
          Offset(plot.left - 9, y - layout.measure('1').height / 2), align: 1);
    }
    final days = layout.window.end.difference(layout.window.start).inDays;
    final labelWidth = layout.measure(days == 1 ? '24:00' : '30 Sep').width + 20;
    final count = (plot.width / labelWidth).floor().clamp(1, days == 1 ? 4 : 7);
    for (var i = 0; i < count; i++) {
      final ratio = count == 1 ? .5 : i / (count - 1);
      if (days == 1 && !activity) {
        final hour = (ratio * 24).round();
        label(canvas, '${hour.toString().padLeft(2, '0')}:00',
            Offset(plot.left + plot.width * ratio, plot.bottom + 12), align: ratio);
      } else {
        final day = count == 1 ? (days / 2).floor() : (ratio * (days - 1)).round();
        final at = layout.window.start.add(Duration(days: day, hours: 12));
        label(canvas, DateFormat('d MMM').format(at.toUtc().add(const Duration(hours: 4))),
            Offset(layout.x(at), plot.bottom + 12), align: .5);
      }
    }
    label(canvas, 'Mauritius time',
        Offset(plot.center.dx, size.height - layout.measure('MUT').height), align: .5);
    if (selected != null) {
      final x = layout.x(points[selected!].at);
      canvas.drawLine(Offset(x, plot.top), Offset(x, plot.bottom),
          Paint()..color = colors.textMuted.withValues(alpha: .55)..strokeWidth = 1);
    }
    canvas.save();
    ...
    for (var i = 0; i < points.length; i++) {
      ...
      if (p.value == null) {
        if (activity) label(canvas, '—', Offset(x, plot.bottom - 24), align: .5);
        continue;
      }
      ...
      if (activity && p.value! > 0) {
        ...
        canvas.drawRRect(RRect.fromRectAndRadius(
          ...
          const Radius.circular(5)), primary);
      } else {
        canvas.drawCircle(Offset(x, y), selected == i ? 6 : 4, primary);
      }
      if (p.secondary != null) {
        final sy = layout.y(p.secondary!);
        canvas.drawLine(Offset(x, y), Offset(x, sy),
            Paint()..color = color.withValues(alpha: .3)..strokeWidth = 1.5);
        canvas.drawRect(Rect.fromCenter(center: Offset(x, sy), width: 7, height: 7), secondary);
      }
    }
    canvas.restore();
    if (focused) {
      canvas.drawRect(plot, Paint()..color = colors.accent
