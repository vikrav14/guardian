...
class WellnessTrend {
  ...
  }) : readings = samples.where((s) =>
        s.metric == metric && window.contains(s.recordedAt, now: now) &&
        (pilotPreview || metric != WellnessMetric.skinTemperature)).toList()
  ...
  int get daysWithReadings => readings
      .map((s) => wellnessDateKey(s.recordedAt)).toSet().length;
...
List<ActivityDay> wellnessActivityDays({
  ...
  for (final day in days) {
    if (!window.includesDate(day.localDate) || day.steps < 0 ||
        !window.contains(day.lastObservedAt, now: now)) continue;
    final previous = byDate[day.localDate];
    if (previous == null || day.lastObservedAt.isAfter(previous.lastObservedAt)) {
      byDate[day.localDate] = day;
    }
  }
  return byDate.values.toList()..sort((a, b) => a.localDate.compareTo(b.localDate));
}
