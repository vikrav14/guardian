...
class _SafetySnapshotPageState extends State<SafetySnapshotPage>
      ...
      if (pending ||
        ...
        setState(() {
          _images.removeWhere(
            (id, _) =>
                !(_feed?.items.any(
                      (item) => item.id == id && item.isViewable,
                    ) ??
                    false),
          );
        });
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      setState(() {
        ...
        _images.removeWhere(
          (id, _) =>
              !feed.items.any((item) => item.id == id && item.isViewable),
        );
      });
    } catch (error) {
      if (mounted) {
  ...
  String _message(Object error) => error is SnapshotFailure
      ? error.message
      : const SnapshotFailure('network').message;
  ...
  Widget build(BuildContext context) {
    final feed = _feed;
    final waiting =
        _sending || (feed?.items.any((item) => item.isPending) ?? false);
    ...
    final explanation =
        ...
        (!_foreground
            ...
            : !feed.online
            ? 'The watch needs to be connected to take a photo.'
            : waiting
            ? 'Waiting for the watch to send one photo. This can take up to two minutes.'
            : cooling
            ? 'Next photo available after ${DateFormat.Hm().format(feed.retryAt!.toLocal())}.'
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'Photos are private and expire after 24 hours. '
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              deleting ? 'Deleting…' : item.statusLabel,
            ...
            if (item.state == SafetySnapshotState.failed)
              const Text(
                'The watch did not provide a complete photo for this request.',
              ),
            if (item.isPending)
              TextButton(
...
class _PrivatePhotoState extends State<_PrivatePhoto> {
  ...
  Widget build(BuildContext context) => Image(
    ...
    height: 300,
    fit: BoxFit.contain,
    semanticLabel: 'Photo received from the watch',
...
class _PhotoPermissionDialogState extends State<_PhotoPermissionDialog> {
  ...
  Widget build(BuildContext context) => AlertDialog(
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Reason for this safety check',
            ),
          ),
          CheckboxListTile(
            ...
            title: const Text(
              'I have the wearer’s or responsible guardian’s permission '
    ...
    actions: [
      ...
      FilledButton(
        onPressed: _confirmed && _purpose.text.trim().length >= 8
            ? () => Navigator.pop(context, _purpose.text.trim())
            : null,
