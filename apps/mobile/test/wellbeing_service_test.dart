...
void main() {
  ...
  for (final edition in ['essential', 'family', 'care']) {
    test(
      '$edition streams accepted readings within its calendar window',
      () async {
        ...
        final result = await WellbeingService(db: db, auth: auth)
            .watchRecentReadings('AAA', subscription: plan(edition), now: now)
            .firstWhere((values) => values.isNotEmpty);
        expect(
          result.map((reading) => reading.id),
          edition == 'essential' ? ['today'] : ['today', 'yesterday'],
        );
      },
    );
  }
