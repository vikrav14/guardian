...
void main() {
  test(
    'a saved check is separate from sensor evidence and disappears on unlink',
    () async {
      ...
      final data = (await ref.collection('wearChecks').doc('current').get())
          .data()!;
      expect(data['recordedBy'], 'owner');
      expect(WearCheck.fromMap(data)?.state, 'worn');
      expect(
        (await ref.collection('wearStatus').doc('current').get()).exists,
        false,
      );
