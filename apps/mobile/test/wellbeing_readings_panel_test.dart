...
void main() {
  ...
  testWidgets('shows exact estimates, freshness and medical boundary', (
    tester,
  ) async {
  ...
  testWidgets('shows the Care boundary without subscribing to readings', (
    tester,
  ) async {
    await tester.pumpWidget(
      app(
        const WellbeingReadingsPanel(allowed: false, readings: Stream.empty()),
      ),
    );
    expect(
      find.text(
  ...
  testWidgets('does not present an old estimate as a current reading', (
    tester,
  ) async {
    ...
    await tester.pump();
    expect(
      find.text('No recent reading · last update 2 h ago'),
      findsOneWidget,
    );
  });
}
