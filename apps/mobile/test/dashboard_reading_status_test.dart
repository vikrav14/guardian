...
void main() {
  ...
  test('active checks need fresh gateway state and a live attempt window', () {
    ...
    expect(
      ReadingPresentation.at(now: start, status: status).title,
      'Checking readings',
    );
    for (final patch in [
      ...
      expect(
        ReadingPresentation.at(now: start, status: {...status, ...patch}).title,
  ...
  test(
    'received result is dated; emphasis expires without claiming wearing',
    () {
      ...
      expect(
        ReadingPresentation.at(
          now: start,
          status: result(at: start.add(const Duration(seconds: 1))),
        ).title,
  ...
  test('unusable optical results suggest fit briefly; timeouts do not', () {
    ...
    final successAfterRemoval = ReadingPresentation.at(
      now: start,
      status: {
        ...result(),
    ...
    expect(
      '${successAfterRemoval.title} ${successAfterRemoval.detail}',
      isNot(contains('worn')),
    );
  });

  test(
    'skipped slots and missing attempts do not become fit or wearing claims',
    () {
      for (final reason in ['watch_offline', 'missed', 'removal_reported']) {
        final view = ReadingPresentation.at(
          now: start,
      ...
      final empty = ReadingPresentation.at(
        now: start,
        status: {
          'phase': 'scheduled',
          'updatedAt': start,
  ...
  testWidgets(
    'quiet streams expire active state and recover after a new result',
    (tester) async {
      ...
      // Deliver the asynchronous event, then render the frame it schedules.
      await tester.pumpAndSettle();
      expect(find.text('Readings received'), findsOneWidget);
      now = now.add(const Duration(minutes: 6));
      tester.binding.handleAppLifecycleStateChanged(AppLifecycleState.resumed);
      await tester.pump();
      final tile = tester.widget<ReadingStatusTile>(
        find.byType(ReadingStatusTile),
      );
  ...
  testWidgets('watch changes and access errors clear previous results', (
    ...
    streams['a']!.add(result());
    streams['b']!.add(
      result(outcome: 'temperature_skipped', reason: 'unusable_oxygen'),
    );
    await tester.pumpAndSettle();
            ...
            body: Padding(
              ...
              child: ReadingStatusTile(
                presentation: ReadingPresentation.at(
                  now: start,
                  status: result(
                    outcome: 'temperature_skipped',
                    reason: 'unusable_oxygen',
                  ),
                ),
                onTap: () => opened = true,
              ),
