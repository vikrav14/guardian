...
Status: **operator-only draft PR #115; both Guardian transitions physically passed on the pilot**.
...
## Guardian physical result — 23 September 2026

The operator ran this helper through Guardian, first Auto, then Manual, using
the same private capture file and reported: **both tests passed with the
expected outcome**. The preceding procedure required an incoming call after
each transition, automatic answering/audio for Auto and answering by touch/audio
for Manual. Record this as operator-confirmed physical acceptance on the
existing V52 pilot and approved caller, not a fleet-wide or factory-state result.

| Guardian request | Supplied machine evidence | Operator-confirmed call result |
| --- | --- | --- |
| Auto | `socket_handoff`, one frame, one session, protocol ID 9705254749 | Expected Auto behavior passed |
| Manual, after Auto | `socket_handoff`, two frames, one session, same protocol ID | Expected Manual behavior passed |

The last tested state is Manual. Exact call timestamps, answer delays and ring
durations were not supplied; do not reuse the earlier reference-run times.
The CLI correctly retains `appliedStateVerified:false`: it cannot observe
the carrier call or independently verify firmware state. Physical acceptance
is recorded here from the operator report rather than fabricated in telemetry.

The implementation commit `f0caa481919d2b727fe9663258d054585c289482` passed
all GitHub release gates (run 331): gateway tests, Firestore authorization,
and Flutter analysis/tests/web build. The local gateway suite passed 1,306 tests.
...
Completed on the pilot: physical Auto and Manual transitions generated by
Guardian, with Manual tested last. Still required: caller exclusivity and
existing unknown-caller rejection while Auto is active; persistence/restart
