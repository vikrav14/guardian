...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. Stationary capture recorded heartbeats without location/radio reports; subsequent `ts#` reports 300 seconds at 44% battery. The passive observer's 60-second maximum match gap and 120-second expiry also cannot support isolated five-minute reports. At the operator's request, a separate one-router native-fence experiment now has preview and explicit one-attempt send, strict-admin/pilot/router/capture/session checks and redacted evidence. It is inferred syntax with unknown rollback, not a hardware pass. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Run the private experiment and inspect response/real behaviour; obtain supplier removal, stationary reporting and current-state recovery semantics. Accept native fencing before normal battery-policy review. Fresh Home app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the real-device acceptance ledger. |
