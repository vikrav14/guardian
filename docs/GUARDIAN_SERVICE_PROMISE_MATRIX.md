...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. A complete 30-minute stationary capture recorded 9 LK + 1 TKQ and no location/radio reports or command handoffs. Subsequent `ts#` reports 300 seconds at 44% battery, consistent with Guardian's normal policy. The passive observer's 60-second maximum match gap and 120-second expiry also cannot support isolated five-minute reports; this is separate from the missing-report gap. Native fence is documented; capture/preview exist, with no native provisioning or timing change enabled. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Obtain supplier single-router/removal, stationary-reporting and current fence-state recovery semantics for the reported V52 firmware; accept native fencing before normal battery-policy review. Fresh Home app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the real-device acceptance ledger. |
