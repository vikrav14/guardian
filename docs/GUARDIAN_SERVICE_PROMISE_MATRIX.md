...
| Home Wi-Fi presence | One V52/Home radio passed near-router recognition on 7 September 2026 UTC. The opt-in display pilot uses the saved Home pin with source/time labels and bounded expiry. Mixed Wi-Fi/cellular packets are supported without treating cellular packets as Home evidence. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Real-device display acceptance, owner-authorized customer enrollment, departure/return and router restart; software guards preserve GPS/SOS/Journey evidence. See `services/wifi-home.md` and the real-device acceptance ledger. |
