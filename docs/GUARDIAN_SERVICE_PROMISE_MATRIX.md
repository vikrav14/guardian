...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. A complete 30-minute stationary capture then recorded 9 LK + 1 TKQ, no location/radio reports or command handoffs, confirming a reporting gap despite check-ins. Native Wi-Fi fence is documented; strict-admin capture and preview exist, but no native provisioning or timing change is enabled. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Read the watch's actual upload interval with proven `ts#`; obtain supplier single-router/removal and stationary-reporting semantics; accept native fencing before normal battery-policy review. Fresh Home app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the real-device acceptance ledger. |
