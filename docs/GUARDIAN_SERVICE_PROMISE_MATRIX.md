...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC, including mixed Wi-Fi/cellular reports. Backend publication passed on 8 September; later screenshots show retained GPS after radio expiry despite recent heartbeats. Native Wi-Fi fence is documented; strict-admin bounded capture and command preview are implemented. No native provisioning or timing change is enabled. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Supplier single-router/removal instructions, native fence and stationary reporting validation, then normal battery-policy review; fresh Home app/map/hero/WhatsApp agreement, customer enrollment, departure/return and router restart. See `services/wifi-home-supplier-validation.md` and the real-device acceptance ledger. |
