...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. The 11 September capture validates named/empty non-GPS scans with nine enrolled-router sightings. A later log confirms two GPS-priority clears, including the clear 6.037 seconds after the last publication renewal; a server Home-zone entry accompanies the second. Home requalifies between switches, then also expires normally during a radio-report gap. Same-packet GPS/radio coexistence and live UI agreement remain unverified. The private diagnostic update passed 780 gateway tests and all CI checks at 20c6144. General activation/enrollment UI remain disabled. | Partial — private pilot | Capture the radio fields of GPS-valid packets, then align any proposed Home/GPS selection across the observer, gateway reader and Flutter. Preserve independent source evidence and verify departure/expiry before enabling a change; keep the current reporting policy, lease and hardware setting during diagnosis. Native acceptance, continuous Home, fresh app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the acceptance ledger. |
