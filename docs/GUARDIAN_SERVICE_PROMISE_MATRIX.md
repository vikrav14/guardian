...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. Continuous Home remains unaccepted; the passive observer's 60-second maximum match gap and 120-second expiry cannot support isolated five-minute reports. The first one-router native experiment completed on 11 September: one setting handoff, no fence reply or event, 21 heartbeats and 13 fresh reports including one Home-radio sighting. Both responses were `CR`, not fence acknowledgements. Native acceptance is inconclusive; setting persistence/removal are unknown. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Preserve the completed trial; do not repeat provisioning. Obtain supplier one-entry/response, readback/removal, stationary reporting and current-state recovery semantics. A supervised transition observation can proceed without another setting, with independent physical markers. Fresh Home app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the real-device acceptance ledger. |
