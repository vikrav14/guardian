...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC, including mixed Wi-Fi/cellular reports. The opt-in display pilot uses the saved Home pin with source/time labels and bounded expiry. A strict-admin runtime check distinguishes publication, expiry and pending I/O; an explicit one-CR check can observe a fresh publication window. General customer activation and enrollment UI remain disabled. | Partial — private pilot | Real-device display acceptance, continuous report acquisition/battery impact, owner-authorized customer enrollment, departure/return and router restart; software guards preserve GPS/SOS/Journey evidence. See `services/wifi-home.md` and the real-device acceptance ledger. |
