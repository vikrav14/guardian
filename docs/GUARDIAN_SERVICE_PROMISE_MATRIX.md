...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. The latest 11 September capture has one CR handoff/reply, ten fresh non-GPS reports and nine enrolled-router sightings at reported -30 dBm. It validates the named/empty scan layout and establishes a fresh baseline; GPS-path physical coverage and publication in this run remain open. The private diagnostic update passed 780 gateway tests and all CI checks at 20c6144. Native fencing has no confirmed setting/transition; the temporary reporting burst still expires under the existing 120-second rule. General activation/enrollment UI remain disabled. | Partial — private pilot | Preserve all captures and the existing setting. Read lastHomePublication, then use fresh router evidence in a short marked radio comparison; review provider eligibility separately. Keep reporting policy and expiry unchanged. Native acceptance, continuous Home, fresh app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the acceptance ledger. |
