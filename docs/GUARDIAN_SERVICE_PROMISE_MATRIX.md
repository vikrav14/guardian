...
| SOS voice messages | The gateway now decodes escaped binary `TK` AMR without corrupting telemetry, accepts only short clips tied to an unresolved SOS, stores them privately, and issues recipient-bound WhatsApp quick replies. The feature flag remains off. | Partial | Approve `guardian_sos_voice_ready_v1`; capture a real V52 message; prove codec/framing, recipient binding, audio playback, expiry, cleanup and data use using the real-device runbook. |
...
| Voice assistant | No verified conversational voice-assistant product exists. SOS voice messages are a separate wearer-recorded safety feature; `MONITOR`/silent listening remains prohibited. | Not implemented | Define a safe conversational product separately and implement it, or remove/reword this promise. |
