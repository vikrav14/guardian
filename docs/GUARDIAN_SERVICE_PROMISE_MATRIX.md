...
| Home Wi-Fi presence | One V52/Home radio passed recognition on 7–8 September 2026 UTC and backend publication passed on 8 September. The 11 September capture has ten fresh non-GPS reports and nine enrolled-router sightings at reported -30 dBm, validating the named/empty scan layout. A later publisher status confirms Home at 19:51:43.682 UTC, cleared 6.037 seconds later and 36.702 seconds before its published expiry; the cause is unconfirmed. This does not prove publication during the earlier capture or UI agreement. The private diagnostic update passed 780 gateway tests and all CI checks at 20c6144. Native fencing has no confirmed setting/transition. General activation/enrollment UI remain disabled. | Partial — private pilot | Preserve the clearing-time gateway reasons before another marked comparison; distinguish GPS priority, contradictory radio evidence and binding changes. Keep reporting policy, expiry and the existing hardware setting unchanged. Native acceptance, continuous Home, GPS-path physical scan coverage, fresh app/map/hero/WhatsApp agreement, enrollment, departure/return and router restart remain open. See `services/wifi-home-supplier-validation.md` and the acceptance ledger. |
