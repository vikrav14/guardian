...
| Home Wi-Fi presence | A disabled, private V52 router observer can collect redacted, source-timed evidence for an operator-selected radio. It has no customer Home inference, enrollment UI or map activation. | Not implemented | Owner-authorized customer enrollment, physical router identity and departure/return evidence, expiry/revocation, clear Home-source presentation and GPS/SOS/Journey preservation; see `services/wifi-home.md`. |
