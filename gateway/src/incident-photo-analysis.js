...
function createPhotoAnalyzer({ apiKey, model, fetchImpl = fetch } = {}) {
  ...
  return async bytes => {
    if (!Buffer.isBuffer(bytes) || bytes.length > 65536) throw new Error('invalid_image');
    ...
    if (payload.stop_reason !== 'end_turn') throw new Error('incomplete_analysis');
    const content = (payload.content || []).filter(part => part.type === 'text').map(part => part.text).join('');
    if (content.length > 5000) throw new Error('invalid_analysis');
    return { ...validateAnalysis(JSON.parse(content)), model, basis: 'original_photo', version: 1 };
  };
}

module.exports = { PROMPT, validateAnalysis, createPhotoAnalyzer };
