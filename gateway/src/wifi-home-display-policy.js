...
const POLICY = 'enrolled_home_radio_v4';
const CONFLICT_POLICY = 'enrolled_home_radio_v3';
...
function evaluateHomeWifiDisplay(observation, binding, nowMs) {
  ...
  // The enrolled radio determines Home proximity. GPS A/V is independent raw
  // telemetry and cannot veto a qualified, unexpired radio match.
  return { reason: 'home_wifi_detected', value: {
    version: 4, policy: POLICY, pilot: true, state: 'matched', source: 'home_wifi',
...
// Saved-pin evidence, never a measured GPS fix or a journey coordinate.
function readHomeWifiEvidence(device, { now = new Date() } = {}) {
  ...
  const previous = value?.version === 3 && value.policy === CONFLICT_POLICY;
  const priority = value?.version === 4 && value.policy === POLICY;
  const conflict = previous && value.state === 'conflict' && CONFLICT_REASONS.has(value.conflictReason);
  if ((!legacy && !spatial && !previous && !priority) || value.pilot !== true ||
  ...
  return { value, legacy, priority, clock, at, expiry, conflict };
}
...
function readHomeWifiDisplay(device, options) {
  ...
  const { value, legacy, priority, clock, at, expiry } = evidence;
  ...
  if (!priority && (legacy ? fixes.some(fix => millis(fix.recordedAt) >= at) :
...
function readHomeWifiConflict(device, options) {
  const evidence = readHomeWifiEvidence(device, options);
  if (!evidence || evidence.legacy || evidence.priority) return null;
...
// Only the new contract may control movement. Cached older presentation records
// retain their original behavior and cannot silently acquire tracking powers.
function readHomeWifiPriority(device, options) {
  const evidence = readHomeWifiEvidence(device, options);
  return evidence?.priority ? evidence.value : null;
}

module.exports = { POLICY, SPATIAL_POLICY, LEGACY_POLICY, MAX_AGE_MS, millis, validAnchor,
  readHomeWifiPriority,
