...
function findEstimatedRouteGaps(
  ...
  for (let index = 0; index < gpsPoints.length - 1; index += 1) {
    ...
    // Approximate observations are corroborating evidence for a sparse
    // interval, not a reason to replace an otherwise well-sampled GPS pair.
...
function routeRequestBody(gap) {
  return {
    origin: {
      location: {
        latLng: {
          latitude: gap.from.lat,
          longitude: gap.from.lng,
        },
      },
    },
    destination: {
...
async function fetchGoogleRouteCandidates(
  ...
  if (!key) {
    throw new Error('A Google Routes API key is required for estimated gaps.');
  }
  if (!gap?.from || !gap?.to) {
    throw new Error('A route gap requires reliable start and end points.');
  }
...
function candidatePlausibility(gap, candidate, metrics) {
  const available = Number(gap.durationSeconds);
  const routeDuration = Number(candidate.durationSeconds);
  const timeLimit = Number.isFinite(available)
    ? Math.max(available * 2, available + 180)
    : null;
  const routeDistance = Number(candidate.distanceMeters);
  const directDistance = Number(gap.directDistanceMeters);
  const distanceLimit = Number.isFinite(directDistance)
    ? Math.max(directDistance * 4, directDistance + 5000)
    : null;

  const reasons = [];
  if (timeLimit != null && Number.isFinite(routeDuration) && routeDuration > timeLimit) {
    reasons.push('route_longer_than_recorded_time_allows');
  }
  if (distanceLimit != null && Number.isFinite(routeDistance) && routeDistance > distanceLimit) {
    reasons.push('route_is_an_extreme_detour');
  }
  if (
    metrics.endpointMaxCorrectionMeters != null &&
    metrics.endpointMaxCorrectionMeters > 500
  ) {
    reasons.push('route_endpoints_are_too_far_from_gps');
  }
  if (
    metrics.approximateMedianDistanceMeters != null &&
...
function selectLikelyGoogleRoute(gap, candidates) {
  const measured = (candidates || []).map((candidate) => {
    ...
    const plausibility = candidatePlausibility(gap, candidate, metrics);
    const approximateScore = metrics.approximateMedianDistanceMeters ?? 500;
    const overrunScore = (metrics.durationOverrunSeconds ?? 0) * 0.5;
    const detourScore = (metrics.detourRatio ?? 1) * 25;
    const defaultBonus = candidate.isDefault ? -15 : 0;
    return {
      candidate,
      metrics,
      ...plausibility,
      score: approximateScore + overrunScore + detourScore + defaultBonus,
    };
  });
  ...
  plausible.sort((left, right) => left.score - right.score);
  const selected = plausible[0];
  const confidence = selected.metrics.approximatePointCount > 0 &&
...
async function estimateRouteGaps(
  ...
  for (const gap of gaps || []) {
    try {
      ...
      output.push({ gap, ...decision, error: null });
    } catch (error) {
      ...
      output.push({
        ...
        error: apiKey
          ? message.replaceAll(String(apiKey), '[redacted]')
          : message,
      });
    }
  }
