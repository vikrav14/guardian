...
function parseTemperatureTrialOperation(payload) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload) ||
      Object.keys(payload).length !== 2 || payload.action !== 'single' ||
      payload.operatorPosition !== 'worn') {
    throw new Error('Use action single with operatorPosition worn for this supervised test.');
  }
...
function createSupervisedTemperatureTrial({ config, readContext, currentSession,
  ...
  async function request(payload) {
    parseTemperatureTrialOperation(payload);
    ...
    try {
      ...
      evidence.start(session, { requestedAt, trialId, operatorPosition: 'worn',
        modeBt: session.wellnessTemperatureMode?.bt ?? session.wellnessLastReportedTemperatureBt ?? null });
      let outcome;
      try {
        const result = send('bodytemp2');
      ...
      return { outcome, trialId, command: 'bodytemp2', requestedAt: requestedAt.toISOString(),
