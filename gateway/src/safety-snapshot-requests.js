...
function assessSnapshotAccess({ requesterUid, user, owner, subscription, imei, now = new Date() }) {
  ...
  if (!entitlements.serviceActive) {
    return { ok: false, reason: entitlements.reason || 'service_inactive' };
  }
  if (!['family', 'care'].includes(entitlements.plan)) {
    return { ok: false, reason: 'family_plan_required' };
  }
  return { ok: true, ownerUid, plan: entitlements.plan };
}

async function processSnapshotRequest(db, requestId, {
  now = new Date(),
  cooldownMinutes = 15,
} = {}) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const access = assessSnapshotAccess({
      requesterUid,
      user,
    ...
    try {
      ...
      tx.set(requestRef, {
        ...
        validationMessage: String(error.message || 'invalid_request').slice(0, 160),
        processedAt: admin.firestore.FieldValue.serverTimestamp(),
      }, { merge: true });
